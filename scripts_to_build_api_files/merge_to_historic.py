...
valid_var_names = era5_land_valid_var_names
files_list = era5_land_files
coord_locs = era5_land_coords_locs

def update_historic_records(data_source, valid_var_names, files_list, coord_locs):
    if data_source != 'fenz' and test_if_all_locs_exist(data_source, valid_var_names, files_list, coord_locs):
        return
    
    if data_source == 'era5' or data_source == 'era5_land':
        months_to_remove = 5
    elif data_source == 'nasapower' or data_source == 'fenz':
        months_to_remove = 1
    else:
    ...
    for hf in hist_files[0:3]:
        ...
        if latest_historic_date < date_to_copy_current_up_to:
            print(f'in {data_source} {hf}')
            cdf_old = cdf# subset cdf so it is only the entries before date_to_copy_current_up_to and check there is still values in it, if not print warning
            # if there is, do below
            # merged_df = pl.concat([cdf_old, hdf], how="vertical")
